...
To get the total 65+ population, **purrr::map_chr()** is used to programmatically generate the list of all male and female senior age bracket variable IDs. The data is then saved locally as an .rds file to avoid repeat API calls.
...
We then clean and merge the three raw datasets. The internet data is pivoted wide, the age data is summarized to sum all 65+ brackets, and percentages are calculated for both. Finally, the three clean data frames are put into a list and joined together using **purrr::reduce()**.
...
ggplot(game_log_clean, aes(x = game_number, y = win_differential)) +
  ...
  labs(
    ...
    caption = "Data: Wikipedia '2025 Seattle Mariners season' page"
  ) +
